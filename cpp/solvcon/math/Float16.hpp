 ...
 *
 * Native compiler types provide conversion when available. Other toolchains
 * use a bitwise round-to-nearest-even fallback.
 */
class Float16
{
...
{
#ifdef SC_FLOAT16_NATIVE_TYPE
    auto const native = static_cast<native_type>(value);
    return std::bit_cast<storage_type>(native);
#else
...
constexpr Float16::storage_type Float16::encode_fallback(T value)
{
    ...
    if (exponent == exp_mask)
    {
        ...
        bits_type const payload = (fraction >> (frac_bits - 10)) | ((fraction >> (frac_bits - 10)) == 0);
