...
/// Handed to Task::execute; stores the result that the task thread reads after execute() returns.
class TaskContext
{
public:
...
private:
    ...
    std::deque<Submission> drain();
    /// Return the result one time, when it exists.
    std::optional<Result> take();
    /// Stop accepting a result and return it when take() has not.
...
private:
    ...
    void start();
    /// Complete queued workflows with Cancelled, join every thread, and emit stopped().
    void shutdown();
