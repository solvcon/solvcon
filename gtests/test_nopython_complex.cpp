...
TEST(Complex32, NumberConversion)
{
    namespace sc = solvcon;
...
TEST(Complex, StdComplexWideningConversion)
{
    namespace sc = solvcon;
    std::complex<float> const source{1.5F, -2.0F};
    sc::Complex<double> const target{source};

    EXPECT_EQ(source.real(), target.real());
    EXPECT_EQ(source.imag(), target.imag());
}
