...
TEST(CellTypeGroupTest, get_instance) {
    ...
    EXPECT_EQ(ctg.size(), index_type(CellTypeGroup::NTYPE));

    // point
    EXPECT_EQ(ctg.point().type()    , 0);
    ...
    // quadrilateral
    EXPECT_EQ(ctg.quadrilateral().type()    , 2);
    ...
    // hexahedron
    EXPECT_EQ(ctg.hexahedron().type()    , 4 );
    ...
    // tetrahedron
    EXPECT_EQ(ctg.tetrahedron().type()    , 5);
